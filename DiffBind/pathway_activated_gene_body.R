...
# in/out parameters
dir_in <- 'input'
dir_out<- 'output_activated_gene_body'

file_name <- 'RTKII_K27ac+K36me3'

# readin files
gic_gain <- as.data.frame(read.table(file.path(dir_in, "RTKII_K27ac_K36_K27ac_RTKII_gic_gain.bed_K36me3_RTKII_v2_gic_gain.bed"),header = FALSE, sep="\t",stringsAsFactors=FALSE, quote=""))
...
loss_gain <- as.data.frame(read.table(file.path(dir_in, "RTKII_K27ac_K36_K27ac_RTKII_gic_loss.bed_K36me3_RTKII_v2_gic_loss.bed"),header = FALSE, sep="\t",stringsAsFactors=FALSE, quote=""))
...
annaData<-getAnnotation(ensembl, featureType = "transcript")
granges_gain <- GRanges(gic_gain)
gain.peaksAnno=annotatePeakInBatch(granges_gain, AnnotationData=annaData, output="overlapping", FeatureLocForDistance="geneEnd", bindingRegion = c(-1, 1))

granges_loss <- GRanges(loss_gain)
loss.peaksAnno=annotatePeakInBatch(granges_loss, AnnotationData=annaData, output="overlapping", FeatureLocForDistance="geneEnd", bindingRegion = c(-1, 1))
